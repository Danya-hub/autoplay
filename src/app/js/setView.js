...
export default ((_index) => {
    ...
    function _getElemFromRange(_dir) {
        ...
        _wasLastClick() ? _findNearDir(_dir) : null;
    }

    function _wasLastClick() {
        return Object.keys(data.point)[Object.keys(data.point).length - 1] == data.wasClicked;
    }

    function _findNearDir(_dir) {
        let coordAxis = data.view.start[_dir][data.view.start[_dir].length - 1];
        let coordCenter = data.view.finish.center;
